...
class QwkGuiApp:
    ...
    def _render_message(self, message_index: int) -> None:
        ...
        # Metadata fields aligned for scanability
        def insert_meta(label, value, tag=None, command=None, is_link=True):
            self.detail_text.insert(tk.END, f"{label:<8} ", "header_meta_label")
            if is_link:
                item_tag = f"{tag}_{id(msg)}"
                self.detail_text.insert(tk.END, value, ("link", "header_meta", item_tag))
                if command:
                    self.detail_text.tag_bind(item_tag, "<Button-1>", command)
            else:
        ...
        insert_meta("Date:", f"{header.msgdate} {header.msgtime}", is_link=False)
        insert_meta(
            "Conf:",
            conf_name,
            "conf_link",
        ...
        if header.msgnum is not None:
            insert_meta("Msg:", str(header.msgnum), is_link=False)
        if msg.refnum:
            insert_meta(
                "Ref:",
                str(msg.refnum),
