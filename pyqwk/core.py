...
def render_msg_links_as_text(msg_link_list: list[dict[str, Any]], use_colors: bool = True) -> str:
    """Render a message links list into a human-readable text report."""
    lines = []
...
def show_list_msg_links(
    ...
    """Read archives and export a summary list of extracted message links."""
    all_messages = []
    ...
    for msg in all_messages:
        if matches_filters(msg, settings, allowed_conferences, user_name, allowed_exclude_conferences):
            extracted_links = [m.group(0) for m in RE_MSG_LINK_PATTERN.finditer(msg.text or "")]
            for link_val in extracted_links:
                link_clean = link_val.strip()
                if not link_clean:
                    continue
    ...
    msg_link_list = []
    for link_val, stats in sorted(msg_link_stats.items(), key=lambda x: (-x[1]["count"], x[0].lower())):
