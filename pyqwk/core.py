...
def render_bbs_as_text(bbs_list: list[dict[str, Any]], use_colors: bool = True) -> str:
    ...
    lines = []
    header_str = "Bulletin Board Systems (BBS)"
    if use_colors:
        lines.append(_colorize(header_str, "bold", "cyan"))
    else:
        lines.append(header_str)
    ...
    col_hdr = f"  {'BBS Name':<28} {'ID':<10} {'Confs':<7} {'Messages':<10} {'First Active':<12} {'Last Active':<12}"
    if use_colors:
        lines.append(_colorize(col_hdr, "bold"))
    else:
        lines.append(col_hdr)
    ...
    for item in bbs_list:
        bbs_name = str(item["bbs_name"])
        if len(bbs_name) > 26:
            bbs_name = bbs_name[:23] + "..."
        bbs_id = str(item.get("bbs_id") or "N/A")
        if len(bbs_id) > 8:
            bbs_id = bbs_id[:8]
        ...
        row_str = f"  {bbs_name:<28} {bbs_id:<10} {confs:<7} {msgs:<10} {first_act:<12} {last_act:<12}"
        lines.append(row_str)
    ...
    summary_str = f"Total BBS Systems: {len(bbs_list)}"
    if use_colors:
        lines.append(_colorize(summary_str, "bold", "green"))
    else:
        lines.append(summary_str)
...
def _render_bbs_html(bbs_list: list[dict[str, Any]], title: str) -> str:
    ...
    html_parts.append(f"<h1>{title}</h1>")
    html_parts.append("<table class='stats-table'>")
    html_parts.append("<thead><tr><th>BBS Name</th><th>ID</th><th>Sysop</th><th>Location</th><th>Conferences</th><th>Messages</th><th>First Active</th><th>Last Active</th></tr></thead>")
    html_parts.append("<tbody>")
    for item in bbs_list:
        html_parts.append(
            f"<tr><td>{html.escape(str(item['bbs_name']))}</td>"
            f"<td>{html.escape(str(item.get('bbs_id') or 'N/A'))}</td>"
            f"<td>{html.escape(str(item.get('sysop') or 'N/A'))}</td>"
            f"<td>{html.escape(str(item.get('location') or 'N/A'))}</td>"
...
def _render_bbs_markdown(bbs_list: list[dict[str, Any]], title: str) -> str:
    md_parts = [f"# {title}\n"]
    md_parts.append("| BBS Name | ID | Sysop | Location | Conferences | Messages | First Active | Last Active |")
    md_parts.append("|----------|----|-------|----------|-------------|----------|--------------|-------------|")
    for item in bbs_list:
        md_parts.append(
            f"| {item['bbs_name']} | {item.get('bbs_id') or 'N/A'} | {item.get('sysop') or 'N/A'} | {item.get('location') or 'N/A'} | {item['conference_count']} | {item['message_count']} | {item['first_active'] or 'N/A'} | {item['last_active'] or 'N/A'} |"
        )
    return "\n".join(md_parts)
...
def _render_bbs_csv(bbs_list: list[dict[str, Any]]) -> str:
    output = io.StringIO()
    writer = csv.DictWriter(output, fieldnames=["bbs_name", "bbs_id", "sysop", "location", "conference_count", "message_count", "first_active", "last_active"])
    writer.writeheader()
...
def show_list_bbs(
    ...
    for input_path in input_paths:
        try:
            ...
            default_name = (bbs_info.name if bbs_info and bbs_info.name else None) or "Unknown"
            default_id = (bbs_info.bbs_id if bbs_info and bbs_info.bbs_id else None) or "N/A"
            ...
            for msg in msgs:
                ...
                if matches_filters(msg, settings, allowed_conferences, user_name, allowed_exclude_conferences):
                    bname = msg.bbs_name or default_name
                    bid = msg.bbs_id or default_id
                    key = (bname, bid)

                    entry = bbs_map[key]
                    entry["bbs_name"] = bname
                    entry["bbs_id"] = bid
                    if bbs_info:
                        if bbs_info.sysop and not entry["sysop"]:
                            entry["sysop"] = bbs_info.sysop
                        if bbs_info.location and not entry["location"]:
                            entry["location"] = bbs_info.location

                    entry["count"] += 1
                    c_identifier = msg.confname or msg.confnum
                    if c_identifier is not None:
                        entry["conferences"].add(c_identifier)

                    msg_dt = msg.datetime
                    if not msg_dt and hasattr(msg, "header") and msg.header and msg.header.msgdate:
            ...
            if not msgs and bbs_info and bbs_info.name:
                key = (bbs_info.name, bbs_info.bbs_id or "N/A")
                entry = bbs_map[key]
                entry["bbs_name"] = bbs_info.name
                entry["bbs_id"] = bbs_info.bbs_id or "N/A"
                entry["sysop"] = bbs_info.sysop or ""
                entry["location"] = bbs_info.location or ""
                for cnum, cname in board_dict.items():
                    entry["conferences"].add(cname or cnum)

        except Exception as e:
            logger.error("Failed to load archive %s: %s", input_path, e)

    bbs_list = []
    for key, stats in sorted(bbs_map.items(), key=lambda x: (-x[1]["count"], x[0][0].lower())):
        first_active_str = stats["first_dt"].strftime("%Y-%m-%d") if stats["first_dt"] else None
        last_active_str = stats["last_dt"].strftime("%Y-%m-%d") if stats["last_dt"] else None

        bbs_list.append({
            "bbs_name": stats["bbs_name"],
            "bbs_id": stats["bbs_id"] or "N/A",
            "sysop": stats["sysop"] or "N/A",
            "location": stats["location"] or "N/A",
    ...
    if not bbs_list:
        logger.warning("No BBS entries found.")
        return
