...
def render_urls_as_text(url_list: list[dict[str, Any]], use_colors: bool = True) -> str:
    ...
    col_hdr = f"  {'URL':<45} {'Messages':<10} {'Authors':<9} {'First Active':<12} {'Last Active':<12} {'BBS Name':<15}"
    if use_colors:
    ...
    for item in url_list:
        ...
        row_str = f"  {url_str:<45} {msgs:<10} {authors:<9} {first_act:<12} {last_act:<12} {bbs:<15}"
    ...
    summary_str = f"Total URLs: {len(url_list)}"
    if use_colors:
...
def _render_csv_table(
    ...
    """Render a list of dictionary records to a CSV string."""
    output = io.StringIO()
...
def show_list_urls(
    ...
    for msg in all_messages:
        if matches_filters(msg, settings, allowed_conferences, user_name, allowed_exclude_conferences):
            extracted_urls = RE_URL_PATTERN.findall(msg.text or "")
            for url in extracted_urls:
                url_clean = url.strip()
                if not url_clean:
                    continue
                stats = url_stats[url_clean]
                stats["count"] += 1
    ...
    url_list = []
    for url_val, stats in sorted(url_stats.items(), key=lambda x: (-x[1]["count"], x[0].lower())):
    ...
    if not url_list:
        logger.warning("No URLs found across messages.")
        return
...
def render_emails_as_text(email_list: list[dict[str, Any]], use_colors: bool = True) -> str:
    """Render an email address list into a human-readable text report."""
    lines = []
    ...
    col_hdr = f"  {'Email':<45} {'Messages':<10} {'Authors':<9} {'First Active':<12} {'Last Active':<12} {'BBS Name':<15}"
    if use_colors:
        lines.append(_colorize(col_hdr, "bold"))
    else:
        lines.append(col_hdr)
    ...
    for item in email_list:
        email_str = str(item["email"])
        if len(email_str) > 43:
            email_str = email_str[:40] + "..."
        msgs = str(item["message_count"])
        authors = str(item["authors_count"])
        first_act = str(item["first_active"] or "N/A")
        last_act = str(item["last_active"] or "N/A")
        ...
        row_str = f"  {email_str:<45} {msgs:<10} {authors:<9} {first_act:<12} {last_act:<12} {bbs:<15}"
        lines.append(row_str)
    ...
    summary_str = f"Total Emails: {len(email_list)}"
    if use_colors:
        lines.append(_colorize(summary_str, "bold", "green"))
    else:
        lines.append(summary_str)
...
def show_list_emails(
    ...
    """Read archives and export a summary list of extracted email addresses."""
    all_messages = []
    allowed_conferences = set()
    allowed_exclude_conferences = set()
    user_name = settings.my_name
    ...
    # 2. Apply settings filters and group by extracted email
    ...
    for msg in all_messages:
        if matches_filters(msg, settings, allowed_conferences, user_name, allowed_exclude_conferences):
            extracted_emails = RE_EMAIL_PATTERN.findall(msg.text or "")
            for email_addr in extracted_emails:
                email_clean = email_addr.strip()
                if not email_clean:
                    continue
                stats = email_stats[email_clean]
                stats["count"] += 1
                msgfrom = (msg.header.msgfrom or "").strip()
                if msgfrom:
                    stats["authors"].add(msgfrom)
    ...
    # 3. Build email list sorted by count descending, then email ascending
    email_list = []
    for email_val, stats in sorted(email_stats.items(), key=lambda x: (-x[1]["count"], x[0].lower())):
        first_active_str = stats["first_dt"].strftime("%Y-%m-%d") if stats["first_dt"] else None
        last_active_str = stats["last_dt"].strftime("%Y-%m-%d") if stats["last_dt"] else None
        bbs_str = ", ".join(sorted(stats["bbs_names"])) if stats["bbs_names"] else "Unknown"
    ...
    if not email_list:
        logger.warning("No email addresses found across messages.")
        return
    ...
    if settings.format == "json":
        ...
        output = render_emails_as_text(email_list, use_colors=use_colors)
...
def render_phones_as_text(phone_list: list[dict[str, Any]], use_colors: bool = True) -> str:
    ...
    for item in phone_list:
        ...
        if len(phone_display) > 42:
            phone_display = phone_display[:39] + "..."
        bbs_display = item["bbs_name"] or "Unknown"
        if len(bbs_display) > 15:
            bbs_display = bbs_display[:12] + "..."
        ...
        row_str = (
            f"  {phone_display:<45} "
            f"{item['message_count']:<10} "
            f"{item['authors_count']:<9} "
            f"{first_str:<12} "
            f"{last_str:<12} "
            f"{bbs_display:<15}"
        )
    ...
    lines.append(f"Total Phone Numbers: {len(phone_list)}")
    return "\n".join(lines)
...
def show_list_phones(
    ...
    # 2. Apply settings filters and group by extracted phone
    ...
    for msg in all_messages:
        if matches_filters(msg, settings, allowed_conferences, user_name, allowed_exclude_conferences):
            ...
            for phone_val in extracted_phones:
    ...
    phone_list = []
    for phone_num, stats in sorted(phone_stats.items(), key=lambda x: (-x[1]["count"], x[0].lower())):
        first_active_str = stats["first_dt"].strftime("%Y-%m-%d") if stats["first_dt"] else None
        last_active_str = stats["last_dt"].strftime("%Y-%m-%d") if stats["last_dt"] else None
        bbs_str = ", ".join(sorted(stats["bbs_names"])) if stats["bbs_names"] else "Unknown"
...
def render_msg_links_as_text(msg_link_list: list[dict[str, Any]], use_colors: bool = True) -> str:
    """Render an internal message link list into a human-readable text report."""
    lines = []
    ...
    col_hdr = f"  {'Message Link':<45} {'Messages':<10} {'Authors':<9} {'First Active':<12} {'Last Active':<12} {'BBS Name':<15}"
    if use_colors:
        lines.append(_colorize(col_hdr, "bold"))
    else:
        lines.append(col_hdr)
    ...
    for item in msg_link_list:
        msg_link_str = str(item["msg_link"])
        if len(msg_link_str) > 43:
            msg_link_str = msg_link_str[:40] + "..."
        msgs = str(item["message_count"])
        authors = str(item["authors_count"])
        first_act = str(item["first_active"] or "N/A")
        last_act = str(item["last_active"] or "N/A")
        ...
        row_str = f"  {msg_link_str:<45} {msgs:<10} {authors:<9} {first_act:<12} {last_act:<12} {bbs:<15}"
        lines.append(row_str)
    ...
    summary_str = f"Total Message Links: {len(msg_link_list)}"
    if use_colors:
        lines.append(_colorize(summary_str, "bold", "green"))
    else:
        lines.append(summary_str)
...
def _render_msg_links_csv(msg_link_list: list[dict[str, Any]]) -> str:
    return _render_csv_table(
        msg_link_list,
        ["msg_link", "message_count", "authors_count", "first_active", "last_active", "bbs_name"],
    )


def show_list_msg_links(
    input_paths: list[str], settings: ProcessingSettings, logger: logging.Logger
) -> None:
    """Read archives and export a summary list of extracted internal message link references."""
    all_messages = []
    allowed_conferences = set()
    allowed_exclude_conferences = set()
    user_name = settings.my_name
    ...
    # 2. Apply settings filters and group by extracted message link reference
    msg_link_stats = defaultdict(lambda: {
        "count": 0,
        "authors": set(),
        "first_dt": None,
        "last_dt": None,
    ...
    for msg in all_messages:
        if matches_filters(msg, settings, allowed_conferences, user_name, allowed_exclude_conferences):
            extracted_msg_links = [m.group(0) for m in RE_MSG_LINK_PATTERN.finditer(msg.text or "")]
            for msg_link_val in extracted_msg_links:
                link_clean = msg_link_val.strip()
                if not link_clean:
                    continue
                stats = msg_link_stats[link_clean]
                stats["count"] += 1
                msgfrom = (msg.header.msgfrom or "").strip()
                if msgfrom:
                    stats["authors"].add(msgfrom)
    ...
    # 3. Build message links list sorted by count descending, then link ascending
    msg_link_list = []
    for msg_link, stats in sorted(msg_link_stats.items(), key=lambda x: (-x[1]["count"], x[0].lower())):
        first_active_str = stats["first_dt"].strftime("%Y-%m-%d") if stats["first_dt"] else None
        last_active_str = stats["last_dt"].strftime("%Y-%m-%d") if stats["last_dt"] else None
        bbs_str = ", ".join(sorted(stats["bbs_names"])) if stats["bbs_names"] else "Unknown"

        msg_link_list.append({
            "msg_link": msg_link,
            "message_count": stats["count"],
            "authors_count": len(stats["authors"]),
            "first_active": first_active_str,
    ...
    if not msg_link_list:
        logger.warning("No message links found across messages.")
        return
    ...
    if settings.format == "json":
        ...
        output = render_msg_links_as_text(msg_link_list, use_colors=use_colors)
...
def render_subjects_as_text(subject_list: list[dict[str, Any]], use_colors: bool = True) -> str:
    ...
    col_hdr = f"  {'Subject':<35} {'Messages':<10} {'Authors':<9} {'First Active':<12} {'Last Active':<12} {'BBS Name':<15}"
    if use_colors:
        lines.append(_colorize(col_hdr, "bold"))
    else:
        lines.append(col_hdr)
    ...
    for item in subject_list:
        ...
        row_str = f"  {subj:<35} {msgs:<10} {authors:<9} {first_act:<12} {last_act:<12} {bbs:<15}"
        lines.append(row_str)
    ...
    summary_str = f"Total Subjects: {len(subject_list)}"
    if use_colors:
        lines.append(_colorize(summary_str, "bold", "green"))
    else:
        lines.append(summary_str)
...
def show_list_subjects(
    ...
    """Read archives and export a summary list of message subjects."""
    all_messages = []
    allowed_conferences = set()
    allowed_exclude_conferences = set()
    user_name = settings.my_name
    ...
    # 2. Apply settings filters and group by subject
    ...
    for msg in all_messages:
        if matches_filters(msg, settings, allowed_conferences, user_name, allowed_exclude_conferences):
            subj = (msg.header.msgsubject or "").strip() or "(No Subject)"
    ...
    # 3. Build subject list sorted by count descending, then subject ascending
    subject_list = []
    for subject, stats in sorted(subject_stats.items(), key=lambda x: (-x[1]["count"], x[0].lower())):
        first_active_str = stats["first_dt"].strftime("%Y-%m-%d") if stats["first_dt"] else None
        last_active_str = stats["last_dt"].strftime("%Y-%m-%d") if stats["last_dt"] else None
        bbs_str = ", ".join(sorted(stats["bbs_names"])) if stats["bbs_names"] else "Unknown"
    ...
    if not subject_list:
        logger.warning("No message subjects found.")
        return
    ...
    if settings.format == "json":
        ...
        output = render_subjects_as_text(subject_list, use_colors=use_colors)
