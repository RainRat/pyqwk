...
def render_urls_as_text(url_list: list[dict[str, Any]], use_colors: bool = True) -> str:
    ...
    col_hdr = f"  {'URL':<40} {'Messages':<10} {'Authors':<9} {'First Active':<12} {'Last Active':<12} {'BBS Name':<15}"
    if use_colors:
        lines.append(_colorize(col_hdr, "bold"))
    else:
        lines.append(col_hdr)
    ...
    for item in url_list:
        ...
        row_str = f"  {url_str:<40} {msgs:<10} {authors:<9} {first_act:<12} {last_act:<12} {bbs:<15}"
        lines.append(row_str)
...
def _render_urls_html(url_list: list[dict[str, Any]], title: str) -> str:
    ...
    for item in url_list:
        ...
        uri = raw_url if "://" in raw_url.lower() else f"http://{raw_url}"
        link_html = f"<a href='{html.escape(uri)}'>{html.escape(raw_url)}</a>"
        html_parts.append(
            f"<tr><td>{link_html}</td>"
            f"<td>{item['message_count']}</td>"
            f"<td>{item['authors_count']}</td>"
            f"<td>{html.escape(str(item['first_active'] or 'N/A'))}</td>"
            f"<td>{html.escape(str(item['last_active'] or 'N/A'))}</td>"
...
def _render_urls_markdown(url_list: list[dict[str, Any]], title: str) -> str:
    ...
    for item in url_list:
        raw_url = item['url']
        uri = raw_url if "://" in raw_url.lower() else f"http://{raw_url}"
        link_md = f"[{raw_url}]({uri})"
        md_parts.append(
            f"| {link_md} | {item['message_count']} | {item['authors_count']} | {item['first_active'] or 'N/A'} | {item['last_active'] or 'N/A'} | {item['bbs_name'] or 'Unknown'} |"
        )
    return "\n".join(md_parts)
...
def show_list_urls(
    ...
    # 2. Apply settings filters and group by extracted URLs
    ...
    for msg in all_messages:
        if matches_filters(msg, settings, allowed_conferences, user_name, allowed_exclude_conferences):
            ...
            for raw_url in found_urls:
                ...
                if not clean_url or clean_url in seen_in_msg:
                    continue
                seen_in_msg.add(clean_url)

                stats = url_stats[clean_url]
                stats["count"] += 1
                msgfrom = (msg.header.msgfrom or "").strip()
                if msgfrom:
                    stats["authors"].add(msgfrom)
