...
def test_format_oneline_highlighting():
    ...
    # Enable highlighting
    oneline = header.format_oneline(board_dict, use_colors=True, highlight_term="Bob")
    assert "\x1b[7mBob\x1b[0m" in oneline

    # Highlight subject
    oneline_sub = header.format_oneline(
        board_dict, use_colors=True, highlight_term="World"
    )
    assert "\x1b[7mWorld\x1b[0m" in oneline_sub

    # Highlight conference
    oneline_conf = header.format_oneline(
        board_dict, use_colors=True, highlight_term="General"
    )
...
def test_process_merged_files_rich_coverage(tmp_path, capsys):
    ...
    msg = ParsedMessage(
        text="This is a test body.", msgnum=1, refnum=None, confnum=1, header=header
    )
...
def test_process_merged_files_body_highlighting(tmp_path, capsys):
    ...
    msg = ParsedMessage(
        text="Find this word.", msgnum=1, refnum=None, confnum=1, header=header
    )
...
def test_fmt_val_no_highlight():
    ...
    formatted = header.format_text(
        board_dict={1: "General"}, verbose=False, highlight_term=None, use_colors=False
    )
