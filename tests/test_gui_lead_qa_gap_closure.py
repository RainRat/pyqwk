...
def test_open_folder_no_archives(app):
    with (
        patch("pyqwk.gui.filedialog.askdirectory", return_value="/tmp/empty"),
...
def test_sort_column_size(app):
    ...
    app.message_list.get_children.side_effect = [
        ["item1", "item2"],
        [],
    ]  # Avoid recursion in traverse
    app.message_list.set.side_effect = lambda k, col: (
        "100 B" if k == "item1" else "20 B"
    )
...
def test_gui_main_entry(monkeypatch):
    ...
    with (
        patch("pyqwk.gui.tk.Tk", return_value=mock_root),
