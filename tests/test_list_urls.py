...
def mock_url_data(message_factory):
    m1 = message_factory(1, 0, "Subject 1", confnum=1, text="Check out https://example.com and http://test.org for details.")
    m1.header.msgfrom = "Alice"
    m1.header.msgto = "Bob"
    m1.datetime = datetime.datetime(2024, 1, 1, 10, 0)

    m2 = message_factory(2, 0, "Subject 2", confnum=1, text="Also see https://example.com for more info.")
    m2.header.msgfrom = "Bob"
    m2.header.msgto = "Alice"
    m2.datetime = datetime.datetime(2024, 1, 2, 11, 0)

    m3 = message_factory(3, 0, "Subject 3", confnum=2, text="No links in this text.")
...
def test_render_urls_formats():
    url_list = [
        {
            "url": "https://example.com/very/long/path/to/some/resource/that/will/be/truncated/in/text",
            "message_count": 10,
            "authors_count": 3,
    ...
    assert "https://example.com/very/long/path/..." in text_out
    assert "http://test.org" in text_out
    assert "Total Unique URLs: 2" in text_out

    # Text format with colors
    text_color_out = render_urls_as_text(url_list, use_colors=True)
    assert "Extracted URLs" in text_color_out
    assert "Total Unique URLs: 2" in text_color_out

    # HTML format
    html_out = _render_urls_html(url_list, "Extracted URLs")
    assert "<h1>Extracted URLs</h1>" in html_out
    assert "<a href='http://test.org'>" in html_out
...
def test_show_list_urls_stdout(mock_url_data):
    ...
    with patch("pyqwk.core.load_data", return_value=(msgs, board)):
        with patch("pyqwk.core._write_text_output") as mock_write:
            ...
            # Sorted by count descending: "https://example.com" (2 msgs), "http://test.org" (1 msg)
            assert urls_out[0]["url"] == "https://example.com"
            ...
            assert urls_out[1]["url"] == "http://test.org"
...
def test_show_list_urls_empty():
    ...
    with patch("pyqwk.core.load_data", side_effect=Exception("Load error")):
        with patch("logging.Logger.warning") as mock_warn:
            ...
            mock_warn.assert_called_with("No URLs found.")
...
def test_show_list_urls_raw_bytes(message_factory):
    m = message_factory(1, 0, "Subject", text="See https://example.com")
    ...
    logger = logging.getLogger("test_url_bytes")
