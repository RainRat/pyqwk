...
def _make_msg(msgnum=1, confnum=1, msgdate='01-01-23', msgtime='12:00',
              msgfrom='Author', msgto='Recipient', msgsubject='Subject',
              text="Body", status=' '):
    header = MessageHeader(
        status=status, msgnum=msgnum, msgdate=msgdate, msgtime=msgtime,
        msgto=msgto, msgfrom=msgfrom, msgsubject=msgsubject,
        msgpassword='', refnum=None, numblocks=1, msgflag='',
        confnum=confnum, lognum=0, nettag=''
    )
    return ParsedMessage(text=text, msgnum=msgnum, refnum=None, confnum=confnum, header=header)

def test_organize_subfolders(tmp_path):
    output_dir = tmp_path / "output"
    output_dir.mkdir()

    msg1 = _make_msg(msgnum=1, confnum=1, msgfrom='Bob', msgto='Alice', msgsubject='Hello')
    msg2 = _make_msg(msgnum=2, confnum=2, msgfrom='Alice', msgto='Bob', msgsubject='Re: Hello')
...
def test_organize_advanced(tmp_path, mocker):
    ...
    msg1 = _make_msg(msgnum=1, confnum=1, msgdate="01-01-23", msgfrom="Bob", msgto="Alice", msgsubject="Hello")
    msg1.confname = "General"
    msg1.bbs_name = "BBS1"

    msg2 = _make_msg(msgnum=2, confnum=1, msgdate="02-01-23", msgfrom="Alice", msgto="Bob", msgsubject="Re: Hello")
    ...
    assert (output_dir / "bbs1" / "bob" / "alice" / "001-general" / "2023" / "01").exists()
    # BBS1 -> Alice -> Bob -> 001-general -> 2023 -> 02
    assert (output_dir / "bbs1" / "alice" / "bob" / "001-general" / "2023" / "02").exists()
...
def test_attachment_prefix_depth(tmp_path, mocker):
    ...
    settings = _make_settings(
        ...
        extract_attachments=True, format="html"
    )
...
def test_relative_attachment_prefix_nested(tmp_path):
    ...
    expected_file = output_dir / "001-general" / "2024" / "05" / "001-00001-subject.html"
...
def test_extract_attachments_dry_run(tmp_path):
    ...
    settings = _make_settings(output_path=str(output_dir), extract_attachments=True, dry_run=True)
