...
def test_filtering_by_recipient_single(
    tmp_path, mock_messages, mock_logger, monkeypatch
):
    output_path = tmp_path / "output.txt"

    monkeypatch.setattr(
        "pyqwk.core.load_data", lambda *args, **kwargs: (bytearray(), {})
    )
    monkeypatch.setattr(
        "pyqwk.core.parse_messages", lambda *args, **kwargs: iter(mock_messages)
    )
...
def test_filtering_by_recipient_multiple(
    tmp_path, mock_messages, mock_logger, monkeypatch
):
    output_path = tmp_path / "output.txt"
...
def test_filtering_by_recipient_case_insensitive(
    tmp_path, mock_messages, mock_logger, monkeypatch
):
    output_path = tmp_path / "output.txt"
