...
def test_organize_by_bbs_success(tmp_path, mock_settings, logger):
    ...
    with (
        ...
        mock_move.assert_called_once_with(
            str(qwk_file), os.path.join("My Awesome BBS", "TEST.QWK")
        )
...
def test_organize_by_bbs_with_id(tmp_path, mock_settings, logger):
    ...
    with (
        patch("pyqwk.core.load_data", return_value=([], board_dict)),
        patch("shutil.move") as mock_move,
        patch("os.makedirs") as mock_makedirs,
    ):
        ...
        mock_move.assert_called_once_with(
            str(json_file), os.path.join(expected_folder, "archive.json")
        )
...
def test_organize_by_bbs_only_id(tmp_path, mock_settings, logger):
    ...
    with (
        ...
        mock_makedirs.assert_called_once_with(expected_folder)
        mock_move.assert_called_once_with(
            str(db_file), os.path.join(expected_folder, "archive.db")
        )
...
def test_organize_by_bbs_exception_handling(tmp_path, mock_settings, logger, caplog):
    ...
    with (
        patch("pyqwk.core.load_data", side_effect=ValueError("Boom")),
        patch("shutil.move") as mock_move,
    ):
...
def test_organize_by_bbs_unsafe_name(tmp_path, mock_settings, logger):
    ...
    with (
        ...
        mock_move.assert_called_once_with(
            str(qwk_file), os.path.join(expected_name, "TEST.QWK")
        )
...
def test_organize_by_bbs_empty_safe_name(tmp_path, mock_settings, logger):
    ...
    with (
        ...
        mock_move.assert_called_once_with(
            str(qwk_file), os.path.join("Unknown_BBS", "TEST.QWK")
        )
...
def test_organize_by_bbs_messages_dat(tmp_path, mock_settings, logger):
    ...
    with (
        ...
        mock_move.assert_called_once_with(
            str(msg_file), os.path.join("BBS", "messages.dat")
        )
