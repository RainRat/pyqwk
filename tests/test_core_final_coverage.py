...
def test_load_data_control_dat_search():
    ...
    with patch("os.path.isdir", return_value=True), \
        ...
        m_open.return_value.__enter__.return_value.read.return_value = b"Produced by pyqwk"
        m_open.return_value.__enter__.return_value.splitlines.return_value = [b"BBS"] * 11
...
def test_generate_safe_filename_extension_check():
    ...
    settings = ProcessingSettings(
        ...
        output_path="out", encoding="cp437", filename_pattern="myname"
    )
...
def test_process_merged_files_invalid_sort(tmp_path):
    ...
    settings = ProcessingSettings(
        ...
        redact_pii=False, format="text", separator="none", output_mode="file",
        output_path=str(tmp_path / "out.txt"), encoding="cp437", sort="invalid", reverse=True
    )
...
def test_write_toc_empty_confs():
    ...
    settings = ProcessingSettings(
        verbose=False, private=True, no_header=False, truncate_signatures=False,
        cut_quoting=False, individual_files=False, threaded=False, binaries_removal=False,
        redact_pii=False, format="html", separator="none", output_mode="file",
        output_path=None, encoding="cp437", include_toc=True
    )
...
def test_write_index_other_format(tmp_path):
    ...
    info = [{'conf_num': 1, 'conf_name': 'Test', 'path': 'msg.md', 'subject': 'Subj', 'from': 'Me', 'to': 'You', 'date': 'now', 'msgnum': 1, 'attachments': []}]
...
def test_show_stats_merged_json():
    ...
    settings = ProcessingSettings(
        ...
        output_path=None, encoding="cp437", merge_stats=True, quiet=True
    )
    logger = logging.getLogger("test")
    # Case 1: format == 'json'
...
def test_process_multiple_files_dry_run():
    ...
    settings = ProcessingSettings(
        ...
        cut_quoting=False, individual_files=False, threaded=False, binaries_removal=False,
        redact_pii=False, format="text", separator="none", output_mode="stdout",
        output_path=None, encoding="cp437", dry_run=True
    )
    logger = logging.getLogger("test")
...
def test_order_messages_no_msgnum():
    ...
    header = MessageHeader(" ", None, "01-01-23", "12:00", "To", "From", "Subj", "", None, 1, " ", 1, 1, " ")
...
def test_threading_cycle_nested_report(caplog):
    ...
    # A -> B -> A
    header1 = MessageHeader(" ", 1, "01-01-23", "12:00", "To", "From", "Subj A", "", 2, 1, " ", 1, 1, " ")
    header2 = MessageHeader(" ", 2, "01-01-23", "12:00", "To", "From", "Subj B", "", 1, 1, " ", 1, 1, " ")
...
def test_organize_by_bbs_exists(tmp_path):
    ...
    settings = ProcessingSettings(
        ...
        cut_quoting=False, individual_files=False, threaded=False, binaries_removal=False,
        redact_pii=False, format="text", separator="none", output_mode="stdout",
        output_path=None, encoding="cp437", dry_run=False
    )
    logger = logging.getLogger("test")
    ...
    with patch("pyqwk.core.load_data", return_value=("data", board_dict)), \
