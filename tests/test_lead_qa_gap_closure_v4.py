...
def test_parse_sqlite_reconstruction(tmp_path):
    ...
    conn.execute("CREATE TABLE messages (conference_number INT, message_number INT, date TEXT, author TEXT, recipient TEXT, subject TEXT, status TEXT, text TEXT, reference_number INT, depth INT, thread_id TEXT, parent_message_number INT, conference_name TEXT, bbs_name TEXT, bbs_id TEXT, source_file TEXT, attachments TEXT)")
    conn.execute("INSERT INTO messages (conference_number, message_number, date, author, recipient, subject, status, text, bbs_name, conference_name, attachments, depth, thread_id, parent_message_number, source_file, bbs_id) VALUES (1, 1, '2024-01-01', 'Me', 'You', 'Sub', ' ', 'Text', 'Test BBS', 'General', '', 0, '', NULL, 'test.qwk', 'TESTID')")
...
def test_parse_sqlite_merge_bbs_info(tmp_path):
    ...
    conn = sqlite3.connect(str(db_path))
    conn.execute("CREATE TABLE messages (conference_number INT, message_number INT, date TEXT, author TEXT, recipient TEXT, subject TEXT, status TEXT, text TEXT, reference_number INT, depth INT, thread_id TEXT, parent_message_number INT, conference_name TEXT, bbs_name TEXT, bbs_id TEXT, source_file TEXT, attachments TEXT)")
    conn.execute("INSERT INTO messages (conference_number, message_number, date, author, recipient, subject, status, text, bbs_name, attachments, depth, thread_id, parent_message_number, source_file) VALUES (1, 1, '2024-01-01', 'Me', 'You', 'Sub', ' ', 'Text', 'From Messages', '', 0, '', NULL, 'test.qwk')")
...
def test_parse_xml_single_message_root():
    root = ET.fromstring('<message><header><msgfrom>Me</msgfrom><confnum>1</confnum></header><text>Hi</text></message>')
...
def test_rfc822_mbox_no_at_in_from():
    header = MessageHeader(" ", 1, "01-01-70", "00:00", "To", "Author Name", "Sub", "", None, None, "", 1, 0, "")
...
def test_calculate_stats_limit(tmp_path, logger):
    ...
    def make_msg(i):
        header = MessageHeader(" ", i, "01-01-70", "00:00", "To", "From", f"Sub {i}", "", None, None, "", 1, 0, "")
    ...
    settings = ProcessingSettings(
        ...
        output_path=None, encoding="latin1", limit=2, quiet=True
    )
...
def test_write_text_oneline_no_colors(capsys):
    import sys
    header = MessageHeader(" ", 1, "01-01-70", "00:00", "To", "From", "Sub", "", None, None, "", 1, 0, "")
    ...
    settings = ProcessingSettings(
        ...
        output_path=None, encoding="latin1", oneline=True, quiet=True
    )
    ...
    assert "Num    Conference   Date           From            To              Subject" in captured.out
