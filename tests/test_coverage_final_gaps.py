...
def test_show_stats_with_attachments_reporting(
    capsys, monkeypatch, default_settings, logger
):
    ...
    monkeypatch.setattr(
        qwk,
        "load_data",
        lambda *args, **kwargs: (bytearray(b"Produced "), {1: "General"}),
    )
...
def test_filename_collision_safety_break_coverage(tmp_path, default_settings, logger):
    ...
    with patch("os.path.exists", side_effect=mock_exists):
        ...
        with patch(
            ...
            return_value=(bytearray(b"Produced "), {1: "General"}),
        ):
...
def test_oneline_html_markdown_text_content_coverage(
    message_factory, default_settings, logger
):
    ...
    settings = replace(
        default_settings, oneline=True, format="html", output_mode="stdout"
    )

    with patch(
        "pyqwk.core.load_data", return_value=(bytearray(b"Produced "), {1: "General"})
    ):
        with patch("pyqwk.core.parse_messages", return_value=[msg]):
...
def test_html_markdown_attachments_no_prefix_coverage(
    message_factory, default_settings
):
