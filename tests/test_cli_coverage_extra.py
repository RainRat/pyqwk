...
def test_individual_files_output_not_a_directory(
    monkeypatch, tmp_path, testdata_dir, capsys
):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["qwk", str(input_file), "--individual-files", "-o", str(not_a_dir)],
    )
...
def test_individual_files_missing_output_path_multiple_inputs(
    monkeypatch, testdata_dir, capsys
):
    ...
    monkeypatch.setattr(
        sys, "argv", ["qwk", str(input1), str(input2), "--individual-files"]
    )
...
def test_individual_files_default_output_path(monkeypatch, testdata_dir, capsys):
    ...
    monkeypatch.setattr(
        sys, "argv", ["qwk", str(input_file), "--individual-files", "--dry-run"]
    )
...
def test_individual_eml_output_not_a_directory(
    monkeypatch, tmp_path, testdata_dir, capsys
):
    input_file = testdata_dir / "messages.dat"
    ...
    monkeypatch.setattr(
        sys, "argv", ["qwk", str(input_file), "--format", "eml", "-o", str(not_a_dir)]
    )
...
def test_invalid_date_format_error(monkeypatch, testdata_dir, capsys):
    ...
    stderr = capsys.readouterr().err
    assert (
        "The date format for '01-01-2023' is invalid. Please use YYYY-MM-DD." in stderr
    )
...
def test_individual_files_mode_setup(monkeypatch, testdata_dir, tmp_path):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["qwk", str(input_file), "--individual-files", "-o", str(output_dir)],
    )
...
def test_merge_mode_error(monkeypatch, testdata_dir, caplog):
    ...
    with monkeypatch.context() as m:
        m.setattr(
            cli, "process_merged_files", MagicMock(side_effect=OSError("Merge failure"))
        )
        with caplog.at_level(logging.ERROR):
            with pytest.raises(SystemExit) as exc:
                main()
