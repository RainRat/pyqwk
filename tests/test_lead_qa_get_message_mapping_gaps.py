...
from pyqwk.core import (
    ProcessingSettings, process_merged_files, ParsedMessage,
    MessageHeader, _get_message_mapping, _write_text
)
import logging

def _make_msg(text, author="Alice", subject="Hello", confnum=1, msgnum=101, status=" ", attachments=None, depth=0):
    h = MessageHeader(
        status=status, msgnum=msgnum, msgdate="01-23-24", msgtime="12:34:56",
        msgto="Bob", msgfrom=author, msgsubject=subject, msgpassword="",
        refnum=None, numblocks=None, msgflag=" ", confnum=confnum,
        lognum=0, nettag=""
    )
    return ParsedMessage(
        text=text, msgnum=msgnum, refnum=None, confnum=confnum,
        header=h, attachments=attachments, depth=depth
    )

def test_get_message_mapping_flags_and_indent():
    # Covers lines 2469, 2471, 2493
    msg = _make_msg(
        "Body", status="*", attachments=["file.txt"], depth=1
    )
