...
def test_show_stats_merged_error():
    ...
    with patch(
        "pyqwk.core.calculate_archive_stats", side_effect=FileNotFoundError("Failed")
    ):
...
def test_gui_open_folder_no_selection(app):
    ...
    with (
        patch("pyqwk.gui.filedialog.askdirectory", return_value=""),
...
def test_gui_open_folder_no_archives(app):
    ...
    with (
        patch("pyqwk.gui.filedialog.askdirectory", return_value="/tmp"),
...
def test_gui_show_stats_with_conferences(app):
    ...
    with (
        patch("pyqwk.gui.calculate_archive_stats", return_value=stats),
