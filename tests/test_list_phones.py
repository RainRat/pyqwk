...
def mock_phone_data(message_factory):
    ...
    m1.header.msgto = "Bob"
    m1.text = "Call me at 555-123-4567 or 555-987-6543"
    ...
    m2.text = "Here is 555-123-4567 again!"
...
def test_render_phones_formats():
    phone_list = [
        {
            "phone": "1-800-555-0199-ext-1234-extremely-long-phone-number",
            "message_count": 10,
            "authors_count": 3,
    ...
    assert "1-800-555-0199-ext-1234-extremely-long-p..." in text_out
    assert "555-0199" in text_out
    ...
    assert "<td>555-0199</td>" in html_out
    ...
    assert "| 555-0199 | 5 | 1 | N/A | N/A | Unknown |" in md_out
    ...
    assert "555-0199,5,1,,," in csv_out
...
def test_show_list_phones_stdout(mock_phone_data):
    ...
    with patch("pyqwk.core.load_data", return_value=(msgs, board)):
        with patch("pyqwk.core._write_text_output") as mock_write:
            ...
            # Sorted by count descending: "555-123-4567" (2 msgs, 2 authors), "555-987-6543" (1 msg, 1 author)
            ...
            assert phones_out[1]["phone"] == "555-987-6543"
...
def test_show_list_phones_raw_bytes(message_factory):
    ...
    m.text = "Call 555-123-4567 for info."
