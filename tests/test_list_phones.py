...
def mock_phone_data(message_factory):
    ...
    m1.header.msgto = "Bob"
    m1.text = "Call me at 800-555-0199 or (555) 123-4567"
    ...
    m2.text = "Reach us at 800-555-0199 for support."
    ...
    m3.text = "No phone numbers in this post."
...
def test_render_phones_formats():
    phone_list = [
        {
            "phone": "+1 (800) 555-0199 extension 12345678901234567890",
            "message_count": 10,
            "authors_count": 3,
...
def test_show_list_phones_stdout(mock_phone_data):
    ...
    with patch("pyqwk.core.load_data", return_value=(msgs, board)):
        with patch("pyqwk.core._write_text_output") as mock_write:
            ...
            # Sorted by count descending: "800-555-0199" (2 msgs, 2 authors), "(555) 123-4567" (1 msg, 1 author)
            assert phones_out[0]["phone"] == "800-555-0199"
            ...
            assert phones_out[1]["phone"] == "(555) 123-4567"
        ...
        with patch("pyqwk.cli.expand_paths", return_value=[str(test_file)]):
            with patch("pyqwk.core.load_data", return_value=(msgs, board)):
                with patch("sys.stdout", new=io.StringIO()) as fake_out:
                    ...
                    assert len(output) == 2
                    assert output[0]["phone"] == "800-555-0199"
    ...
    with patch("pyqwk.core.load_data", return_value=([m1], board)):
        with patch("pyqwk.core._write_text_output") as mock_write:
            with patch("pyqwk.core._parse_qwk_date", return_value=None):
                ...
                assert len(out) == 1
                assert out[0]["phone"] == "800-555-0199"
