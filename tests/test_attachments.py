...
def test_process_merged_files_with_attachments():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        message = ParsedMessage(
            text=msg_text, msgnum=1, refnum=None, confnum=1, header=header
        )
...
def test_collision_avoidance():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        settings = _make_settings(
            output_mode="file", output_path=output_path, extract_attachments=True
        )
...
def test_attachment_base_directory_logic(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            settings = _make_settings(
                output_mode="file", output_path=file_path, extract_attachments=True
            )
            pyqwk.core.process_merged_files(["mock.qwk"], settings, MagicMock())
            assert os.path.exists(os.path.join(tmpdir, "attachments", "file.txt"))
            ...
            settings = _make_settings(
                output_mode="stdout", output_path=None, extract_attachments=True
            )
            pyqwk.core.process_merged_files(["mock.qwk"], settings, MagicMock())
            assert os.path.exists(os.path.join(tmpdir, "attachments", "file.txt"))
