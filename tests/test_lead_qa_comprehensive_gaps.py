...
def test_calculate_archive_stats_limit_zero(tmp_path, logger):
    ...
    msg = _message_to_dict(
        ParsedMessage(
            ...
            1,
            MessageHeader(
                " ", 1, "01-01-70", "00:00", "B", "A", "S", "", None, None, "", 1, 0, ""
            ),
        )
    )
...
def test_calculate_archive_stats_skip_and_reply_regex(tmp_path, logger):
    ...
    def make_msg(i, subject):
        h = MessageHeader(
            " ", i, "01-01-70", "00:00", "B", "A", subject, "", None, None, "", 1, 0, ""
        )
    ...
    assert (
        stats["reply_count"] == 2
    )  # "Re: Reply" and "fw: Forward" should be detected via regex
...
def test_write_html_toc_no_bbs_info(monkeypatch):
    captured = []
    monkeypatch.setattr(
        "pyqwk.core._write_text_output", lambda c, p, encoding: captured.append(c)
    )
