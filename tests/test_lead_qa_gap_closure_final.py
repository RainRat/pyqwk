...
from pyqwk.core import ParsedMessage, MessageHeader, _reconstruct_archive_information
...
def test_reconstruct_archive_information_upgrade_default_name():
    ...
    m1 = ParsedMessage(
        text="Body", msgnum=1, refnum=None, confnum=42, header=h1, confname=None
    )

    # Message 2 has a descriptive name
    m2 = ParsedMessage(
        text="Body", msgnum=2, refnum=None, confnum=42, header=h1, confname="The Answer"
    )
...
def test_gui_render_overlapping_entities(mock_gui):
    ...
    mock_gui.messages = [
        ParsedMessage(text=body2, msgnum=2, refnum=None, confnum=1, header=header)
    ]
    mock_gui._render_message(0)
    ...
    body_inserts = [
        c[0][1] for c in mock_gui.detail_text.insert.call_args_list if c[0][1] == ""
    ]
