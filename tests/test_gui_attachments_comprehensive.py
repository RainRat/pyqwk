...
def test_save_attachment_missing_text(mock_gui_deps):
    ...
    app.messages = [ParsedMessage(text="", msgnum=1, refnum=None, confnum=1, header=header)]
    app.message_list.selection.return_value = ["0"]
...
def test_save_attachment_invalid_index(mock_gui_deps):
    ...
    app.messages = [ParsedMessage(text="begin 644 t.txt\n`\nend", msgnum=1, refnum=None, confnum=1, header=header)]
    app.message_list.selection.return_value = ["0"]
...
def test_save_attachment_exception_handling(mock_gui_deps):
    ...
    with patch("builtins.open", side_effect=mock_open):
        ...
        assert "Mock Permission Denied" in str(mock_gui_deps["messagebox"].showerror.call_args)
...
def test_extract_all_attachments_cancelled_folder(mock_gui_deps):
    root = MagicMock()
    app = QwkGuiApp(root)
    app.messages = [ParsedMessage(text="body", msgnum=1, refnum=None, confnum=1, header=MagicMock())]
...
def test_extract_all_attachments_missing_text(mock_gui_deps, tmp_path):
    root = MagicMock()
    app = QwkGuiApp(root)
    app.messages = [ParsedMessage(text="", msgnum=1, refnum=None, confnum=1, header=MagicMock())]
    setup_tree_mock(app, ["0"])
    mock_gui_deps["filedialog"].askdirectory.return_value = str(tmp_path)
...
def test_extract_all_attachments_collision_and_empty_name(mock_gui_deps, tmp_path):
    ...
    app.messages = [
        ParsedMessage(text=text_collision, msgnum=1, refnum=None, confnum=1, header=header),
        ParsedMessage(text=text_empty_name, msgnum=2, refnum=None, confnum=1, header=header)
    ]
