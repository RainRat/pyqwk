...
def test_attachment_filter_logic():
    ...
    msg_no_attach = ParsedMessage(
        text="Hello world", msgnum=2, refnum=None, confnum=1, header=header
    )
...
def test_attachment_metadata_in_csv():
    ...
    captured_content = []
    pyqwk.core._write_text_output = lambda content, *a, **k: captured_content.append(
        content
    )
...
def test_has_attachments_cli_integration(monkeypatch, capsys):
    ...
    msg2 = ParsedMessage(
        text="Just text", msgnum=2, refnum=None, confnum=1, header=header
    )
