...
from pyqwk.core import (
    _parse_html_messages, load_data, _render_stats_html, _render_stats_markdown,
    _compute_stats_from_messages, _order_messages_by_thread, ParsedMessage, MessageHeader
)
...
def test_order_messages_by_thread_circular_reference_complex():
    def make_msg(num, ref):
        h = MessageHeader(
            status=' ', msgnum=num, msgdate='01-01-23', msgtime='10:00',
            msgto='All', msgfrom='User', msgsubject='Cycle', msgpassword='',
            refnum=ref, numblocks=1, msgflag='', confnum=1, lognum=num, nettag=''
        )
    ...
    assert any("Circular reference detected" in call[0][0] for call in mock_logger.warning.call_args_list)
