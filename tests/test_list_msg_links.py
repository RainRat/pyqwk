...
@pytest.fixture
def mock_msg_links_data(message_factory):
    m1 = message_factory(1, 0, "Subj 1", confnum=1)
    m1.header.msgfrom = "Alice"
    m1.header.msgto = "Bob"
    m1.text = "Check out msg #123 and message 456 for details!"
    ...
    m2.text = "I agree, msg #123 was great!"
    ...
    m3.text = "No internal message references here at all."
...
def test_render_msg_links_formats():
    msg_link_list = [
        {
            "msg_link": "msg #123456789012345678901234567890123456789012345",
            "message_count": 10,
            "authors_count": 3,
    ...
    assert "msg #12345678901234567890123456789012345..." in text_out
    assert "msg #123" in text_out
    ...
    assert "<td>msg #123</td>" in html_out
    ...
    assert "| msg #123 | 5 | 1 | N/A | N/A | Unknown |" in md_out
    ...
    assert "msg #123,5,1,,," in csv_out


def test_show_list_msg_links_stdout(mock_msg_links_data):
    msgs, board = mock_msg_links_data
    ...
    with patch("pyqwk.core.load_data", return_value=(msgs, board)):
        with patch("pyqwk.core._write_text_output") as mock_write:
            ...
            # Sorted by count descending: "msg #123" (2 msgs, 2 authors), "message 456" (1 msg, 1 author)
            assert links_out[0]["msg_link"] == "msg #123"
            ...
            assert links_out[1]["msg_link"] == "message 456"
...
def test_cli_list_msg_links_integration(tmp_path, mock_msg_links_data):
            ...
            with patch("pyqwk.cli.expand_paths", return_value=[str(test_file)]):
                with patch("pyqwk.core.load_data", return_value=(msgs, board)):
                    with patch("sys.stdout", new=io.StringIO()) as fake_out:
                        ...
                        assert len(output) == 2
                        assert output[0]["msg_link"] == "msg #123"
                        assert output[0]["message_count"] == 2


def test_show_list_msg_links_all_formats(mock_msg_links_data):
    msgs, board = mock_msg_links_data
...
def test_show_list_msg_links_raw_bytes(message_factory):
    ...
    m.text = "Check msg #100 for details."
