...
def mock_msg_link_data(message_factory):
    ...
    m1.header.msgto = "Bob"
    m1.text = "Please refer to msg #123 or message 456 for details."
    ...
    m2.text = "Regarding msg #123, I agree!"
...
def test_render_msg_links_formats():
    msg_link_list = [
        {
            "msg_link": "msg #1234567890123456789012345678901234567890123",
            "message_count": 10,
            "authors_count": 3,
    ...
    assert "msg #1234567890123456789012345678901234..." in text_out
    assert "message 456" in text_out
    ...
    assert "<td>message 456</td>" in html_out
    ...
    assert "| message 456 | 5 | 1 | N/A | N/A | Unknown |" in md_out
    ...
    assert "message 456,5,1,,," in csv_out
...
def test_cli_list_message_links_alias_integration(tmp_path, mock_msg_link_data):
    test_file = tmp_path / "dummy.qwk"
    test_file.touch()

    msgs, board = mock_msg_link_data

    test_args = ["qwk.py", str(test_file), "--list-message-links", "--format", "json"]
...
def test_show_list_msg_links_raw_bytes(message_factory):
    ...
    m.text = "Check msg #99 for context."
