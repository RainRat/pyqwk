...
def test_tar_extraction_error():
    ...
    with tempfile.NamedTemporaryFile(suffix=".tar") as tmp:
        ...
        with patch("pyqwk.core.tarfile.is_tarfile", return_value=True):
            with patch("pyqwk.core.tarfile.open", side_effect=Exception("Mock extraction error")):
...
def test_tar_bbs_info_name_merging():
    ...
    with tempfile.NamedTemporaryFile(suffix=".tar") as tmp_tar:
        ...
        with patch("pyqwk.core.expand_paths") as mock_expand:
            ...
            msg1 = ParsedMessage(header=create_dummy_header(msgfrom="A"), text="T", confnum=1, msgnum=1, refnum=0)
            cmap1 = ConferenceMap()
            cmap1.bbs_info = BBSInfo()
            cmap1.bbs_info.name = "" # Empty name

            msg2 = ParsedMessage(header=create_dummy_header(msgfrom="B"), text="T", confnum=1, msgnum=2, refnum=0)
            cmap2 = ConferenceMap()
...
def test_tar_classic_qwk_inside():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("pyqwk.core.load_data") as mock_load:
            ...
            mock_msg = ParsedMessage(header=create_dummy_header(confnum=1), text="Hi", confnum=1, msgnum=1, refnum=0)
    ...
    with patch("pyqwk.core.tarfile") as mock_tar_mod:
        ...
        with patch("os.path.isfile", return_value=True):
            ...
            with patch("pyqwk.core.expand_paths", return_value=["p1"]):
                with patch("pyqwk.core.load_data", return_value=([ParsedMessage(header=create_dummy_header(), text="", confnum=1, msgnum=1, refnum=0)], ConferenceMap())):
