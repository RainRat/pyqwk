...
**Key Features:**
- **Search:** Find messages by keyword or use regular expressions for advanced patterns. Cycle through matches with **F3** or **Shift + F3**. The reader automatically moves to the next or previous message when you reach the end of the current one. You can also right-click highlighted text to search for it instantly.
- **Attachments:** Save files by clicking links in the message header. Use **File > Extract All Attachments...** to save all files from your current view.
...
- **Viewing Options:** Hide signatures, quotes, and attachments with the "Clean" view. Use "Remove Colors" to strip color codes. To include images directly in messages, use "Embed Attachments".
